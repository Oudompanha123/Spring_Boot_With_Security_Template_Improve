...
111 tests against in-memory H2 (`src/test/resources/application.yml` shadows the main config, so
...
| `BookAuthorizationIntegrationTest` (14) | the full matrix, including that 401s and 403s are JSON with the right code and that `DELETE` as ADMIN is 204 with an empty body |
...
Three files make this deployable: `Dockerfile`, `.dockerignore`, and `render.yaml` (a blueprint
describing one web service plus a managed Postgres).
...
3. Render asks for the two values marked `sync: false`:
   ...
   - `FILE_BASE_URL` — the service's own public URL, e.g. `https://spring-app.onrender.com`.
4. Deploy. `JWT_SECRET` is generated by Render (`generateValue: true`) and never touches the repo;
   the database host, port, name, user and password are wired from the Postgres resource.

### Five things that would have broken, and how they are handled

| | Fix |
|---|---|
| Render injects `PORT`; the app read `SERVER_PORT` | `port: ${PORT:${SERVER_PORT:8088}}` |
| A managed database exposes host/port/name, and its "connection string" is a `postgres://` URI the JDBC driver rejects | the prod profile composes `jdbc:postgresql://${DB_HOST}:${DB_PORT}/${DB_NAME}`, with `DB_URL` still winning if set |
| `ddl-auto: validate` fails against an empty database | `SPRING_JPA_HIBERNATE_DDL_AUTO=update` for the first deploy — see the warning below |
| No endpoint for the platform to probe | `spring-boot-starter-actuator`, with **only** `/actuator/health` exposed and permitted, reporting `UP` with no component detail |
| Five prod variables have no defaults, so a missing one is a failed startup | all of them are set in `render.yaml`, which is why that file has to be complete |

TLS termination was already handled: `forward-headers-strategy: framework` makes the app trust
Render's proxy headers, so it knows the request arrived over HTTPS.

### Before this carries real data

- **Take `ddl-auto` off `update`.** It is a bootstrap convenience. Hibernate adds columns and never
  removes or alters them, so the schema quietly diverges from the entities and no change is ever
  reviewed. Add Flyway, then set it back to `validate`.
- **Uploads do not survive.** The container filesystem is recreated on every deploy and restart;
  `FILE_SERVER_PATH` points at `/tmp/uploads/`. Anything that must persist needs object storage or
  a Render disk.
- **Free instances sleep.** The first request after idle pays a cold start, which for a JVM is tens
  of seconds — long enough that a health check can fail before the app answers. Check Render's
  current free-tier limits, including how long a free Postgres lives.
- **Rotate `JWT_SECRET` deliberately.** Rotating invalidates every live token, which is correct but
  logs everyone out.
...
curl -s $BASE/api/v1/books                          # 200, public
curl -s -o /dev/null -w '%{http_code}
'      -X POST $BASE/api/v1/books                     # 401, JSON not HTML
curl -s -X POST $BASE/api/v1/auth/signup   -H 'Content-Type: application/json'   -d '{"email":"you@example.com","username":"you","password":"Str0ngPassw0rd"}'
```

Signup is the real check: it exercises the database, the schema and BCrypt in one request.
...
- **No schema migrations.** `ddl-auto: update` in dev, `validate` in prod, and nothing in between.
  The Render blueprint sets `update` for the first deploy, which is a bootstrap shortcut and not a
  way to run a service.
- **The Docker image has not been built.** `Dockerfile` and `render.yaml` are written but unverified
  — there was no Docker daemon available. The jar itself was run under the prod profile with
  Render's exact environment contract against an empty database, so the configuration is proven;
  the image build is not.
